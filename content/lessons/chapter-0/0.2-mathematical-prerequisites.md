...
A **set** is a collection of distinct objects. Sets are fundamental to probability because events are sets of outcomes.
...
**Combinatorics** is the study of counting arrangements and selections. It's essential for calculating probabilities in discrete settings.
...
This will be important when we study the binomial distribution!
...
The **limit** of $f(x)$ as $x$ approaches $a$ is $L$: $\lim_{x \to a} f(x) = L$
...
The **integral** $\int_a^b f(x) dx$ measures the area under the curve.
...
Important for moment generating functions!
