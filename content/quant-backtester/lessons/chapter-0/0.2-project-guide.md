...
This guide walks you through the **"Steps to Complete the Project"** from the [project overview](/quant-backtester/lessons/0.1). Each step is framed as a **task** with **hints** to point you in the right direction. Full code and "raw" answers are hidden in **Show Solution** accordions—try the step yourself first, then expand only when you need to.
...
**P&L and slippage:** We use close‑to‑close returns and no slippage here. For these assumptions and how to add a simple slippage model later (e.g., fixed bps per trade), see [P&L and slippage](/quant-backtester/lessons/0.1#hints-with-backlinks) in the project overview.
